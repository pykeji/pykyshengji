...
<div id="waibig">
	...
	<div id="dtcfcd">
	  <ul>
		<?php for($i = 0;$i<count($zy_yp);$i++){ echo '<li class="dtcfli">'.$zy_yp[$i][0][cf_name].'</li>'; } ?>
	   </ul>
	</div>
	...
	<div id="left_big">
		...
		<!-- 处方明细div -->
		<div id="cfmx">
		<?php for($j = 0;$j<count($zy_yp);$j++){ ?>
		<span class="wtf">
		<?php $numdr = 1; ?>
		 <?php if(is_array($zy_yp[$j])): foreach($zy_yp[$j] as $key=>$vo): ?><div class="zykf_yp">
				<div class="yp1">
		...
		$(document).on('mouseover','.zydrugLi',function(){
		 ...
		 $('.wtf').each(function(){
            ...
            success:function(dd)
            {
               ...
               var wrp = dd.xw1;
               $(sss).eq(nua).next('input').val(wrp);
               $(sss).eq(nua).next('input').next('input').val(code);
            },
            error:function()
            {
                alert('Ajax请求失败');
